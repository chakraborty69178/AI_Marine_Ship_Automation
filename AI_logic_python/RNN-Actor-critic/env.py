...
class Env_tsp():
	def __init__(self, cfg):
		...
		self.shipLocation = shipLocationFile.values[:,0]
		self.portsLocation = portsLocationFile.values
		
			
		self.batch = cfg.batch
		#self.city_t = cfg.city_t
		self.city_t = 11 #len(self.portsLocation[:,0])+1
		coordinates = []
		print(self.portsLocation)
		for i in range(len(self.portsLocation[:,0])+1):
			if(i==0):
				coordinate1 = self.shipLocation[0].split("||")
				c = [float(coordinate1[0]),float(coordinate1[1])]
				coordinates.append(c)
			else:
				coordinate1 = self.portsLocation[i-1,1].split("||")
				c = [float(coordinate1[0]),float(coordinate1[1])]
		    	#coordinates.append(c)
		print(coordinates)
	...
	def get_nodes(self, seed = None):
		...
		#print(torch.rand((self.city_t, 2), device = device))
		#return torch.rand((self.city_t, 2), device = device)
		
		return torch.Tensor([[445.397,338.3717],[732.6194,1065.858],[-35.30655,515.641],[95.92686,1439.007],[-306.2105,1557.319],[168.1906,472.7632],[1568.617,590.7963],[457.7288,637.1407],[780.7388,457.4514],[1071.853,414.8448],[572.8994,919.3412]])
	...
	def stack_l_fast(self, inputs, tours):
		...
		*** this function is faster version of stack_l! ***
		...
		"""
		d = torch.gather(input = inputs, dim = 1, index = tours[:,:,None].repeat(1,1,2).type(torch.int64))
	...
	def show(self, nodes, tour):
		...
		plt.plot(nodes[np_tour.type(torch.LongTensor), 0], nodes[np_tour.type(torch.LongTensor), 1], 'k-', linewidth = 0.7)
